        ...
        console.log(data);
            ...
            $scope.get_recent_date = function() {
                ...
                if (!findValue(new Date())) {
                    ...
                    fw = angular.copy($scope.totalwater($scope.fid))
                }
            ...
            $scope.savlocSQL = function() {
                ...
                $cordovaSQLite.execute(db, query, ['admin']).then(function(res) {
                    if (res.rows.length == 0) {
                        ...
                        $cordovaSQLite.execute(db, insert, ['admin', JSON.stringify($scope.basedata)]).then(function(res) {}, function(err) {
                            console.error(err);
                        });

                    } else {
                        var updata = "UPDATE userd SET basedata = ? WHERE username = ?";
                        $cordovaSQLite.execute(db, updata, [JSON.stringify($scope.basedata), 'admin']).then(function(res) {}, function(err) {
                            console.error(err);
                        });
                    }
                }, function(err) {
            ...
            $scope.set_food_Firebase = function(id) {
                ...
                if (user != null) {
                    ...
                    angular.forEach(food, function(v, k) {
                        ...
                        if (k === 'fitem')
                            angular.forEach(tmp, function(v_i, v_k) {
                                if (v.fd_details != null)
                                    if (v_i.fd_details.length < 1)
                                        v_i.fd_details = 'null';
                            });

                        if (k === 'fdate')
                            tmp = v.getTime();
                        firebase.database().ref(starCountRef + userId + '/food_list/' + id + '/' + k).set(tmp)
                    })
                }
            }
            ...
            $scope.remove_food_Firebase = function(id) {
                var user = firebase.auth().currentUser;
                if (user != null) {
                    var userId = user.uid;
                    var fitem = firebase.database().ref('user_data/' + userId + '/food_list/' + id);
                    fitem.remove()
                        .then(function() {
                            var ref = firebase.database().ref('user_data/' + userId + '/food_list/');
                            ref.once('value').then(function(data) {
                                if (data.exists()) {
                                    firebase.database().ref('user_data/' + userId + '/food_list/').set({})
                                    for (var i = 0; i < data.val().length; i++) {
                                        firebase.database().ref('user_data/' + userId + '/food_list/' + i).set($scope.food_list[i])
                                    };
                                }
                            });
                        })
                        ...
                        if (data.exists()) {
                            $scope.$apply(function() {
                                $scope.basedata = {
                                    ...
                                    food_list: data.child('food_list').val() || []
                                };

                                angular.forEach($scope.basedata.food_list, function(v, k) {
                                    angular.forEach(v.fitem, function(v_i, v_k) {
                                        if (v_i.fd_details === 'null')
                                            v_i.fd_details = [];
                                    });
                                    v.fdate = new Date(v.fdate);
                                })
            ...
            $scope.select_food_Firebase = function() {
                ...
                if (user != null) {
                    var userId = user.uid
                    var starCountRef = firebase.database().ref('user_data/' + userId + '/food_list/');
                    starCountRef.once('value').then(function(data) {
                        angular.forEach(data.val(), function(v, k) {
                            if ($scope.basedata.food_list != null) {
                                var tmp = v;
                                tmp.fdate = new Date(tmp.fdate);
                    ...
                    buttons: [
                        { text: '返回' }, {
                            ...
                            onTap: function(e) {
                                ...
                                var name = t.fname;
                                if (name && t.fdate && t.fkcal != null && t.fwater != null && (t.fkcal || t.fwater) && findName(t.fdate, name)) {
                                    t = {
                                        fdate: t.fdate,
                                        fitem: [{
                                            name: name,
            ...
            $scope.modify_easyfood = function(fid, fitemid) {
                $scope.easy_food = {
                    ...
                    fwater: $scope.basedata.food_list[fid].fitem[fitemid].water
                }
                    ...
                    buttons: [
                        { text: '返回' }, {
                            ...
                            onTap: function(e) {
                                ...
                                if (name && t.fdate && t.fkcal != null && t.fwater != null && (t.fkcal || t.fwater) && ((list.fdate === t.fdate && list.fitem[fitemid].name === name) || findName(t.fdate, name))) {

                                    list.fitem[fitemid].name = name;
                                    list.fitem[fitemid].kcal = $scope.easy_food.fkcal;
                                    list.fitem[fitemid].water = $scope.easy_food.fwater;
            ...
            var savefdlist = function(farray) {
                ...
                findName(farray.fdate, farray.fitem[0].name);
                $scope.set_food_Firebase($scope.fid);
            }
            ...
            $scope.go_addfooditem_2 = function() {
                $scope.addfoodst = null;
                $scope.fdback = 'fooditem';
                $scope.go_addfooditem();
            };
                ...
                angular.forEach($scope.fd, function(v_f, k_f) {
                    angular.forEach(v_f.item, function(v_i, k_i) {
                        if (v_i.id === id) {
                            ...
                            t.basiswater = v_i.Water;
                            t.kcal = t.basiskcal * t.i;
                            t.water = t.basiswater * t.i;
                            return;
                        }
                    });
                });
            ...
            $scope.savefd_details = function() {
                ...
                if ($scope.addfoodst != null) {
                    savefdlist($scope.addfoodst);
                }
                ...
                if (i) {
                    if ($scope.modify_id != null && item.fd_details[$scope.modify_id].name === t.name) {
                        ...
                        item.fd_details[$scope.modify_id].water = t.basiswater;
            ...
            $scope.totalitemwater = function(fid, fitemid) {
                var t = 0;
                angular.forEach($scope.basedata.food_list[fid].fitem[fitemid].fd_details, function(v, k) {
                    t += (v.water * v.Quantity);
                });
                ...
                confirmPopup.then(function(res) {
                    if (res) {
                        ...
                        if ($scope.basedata.food_list[fid].fitem.length < 1) {
                            $scope.basedata.food_list.splice(fid, 1);
                            $scope.remove_food_Firebase(fid);
                        } else
                            $scope.set_food_Firebase(fid);
                    }
                });
            ...
            $scope.modify_fooditem = function(name) {
                ...
                $scope.fdetails_bank = {
                    ...
                    basiswater: details.water,
                    i: details.Quantity,
                    kcal: details.kcal * details.Quantity,
                    water: details.water * details.Quantity,
                };
            ...
            $scope.searchfd = function() {
                $scope.searchfdarray.length = 0;
                if ($scope.searchfdname.name) {
                    angular.forEach($scope.fd, function(value, key) {
                        angular.forEach(value.item, function(v_i, v_k) {
                            if (v_i.name.match($scope.searchfdname.name)) {
                                $scope.searchfdarray.push(v_i);
                            }
                        });
                    });
                }
