 ...
 /*     $location.hash(id);
  ...
  // -----------------  add_Sport  ----------------- 
    ...
    $scope.edit_plan = function(k , pid) {
        $scope.selectSp = {
            pname : $scope.basedata.plan_list[k][pid].pname,
            pdate : $scope.basedata.plan_list[k][pid].pdate,
            pbr_in : $scope.basedata.plan_list[k][pid].pbr_in,
            p_total_mets : $scope.basedata.plan_list[k][pid].p_total_mets,
            psplist : $scope.basedata.plan_list[k][pid].psplist,
        }
        var myPopup = $ionicPopup.show({
          cssClass: '',
          templateUrl: 'templates/activity/addS.html',
    ...
    $scope.edit_activity = function(aid) {
        ...
        $scope.search.name = null;
        for (var i = 0; i < $scope.sl.length; i++) {
          if ($scope.sl[i].name === $scope.basedata.plan_list[k][pid].psplist[aid].sname) {
            $scope.sportoption.s = $scope.sl[i];
            return true;
          };
        };
        $scope.timedata.start = $scope.basedata.plan_list[k][pid].psplist[aid].sstart;
        $scope.timedata.end = $scope.basedata.plan_list[k][pid].psplist[aid].send;
        $scope.timedata.total = $scope.basedata.plan_list[k][pid].psplist[aid].sfeature.cost;
        for (var i = 0; i < $scope.sl_f.length; i++) {
          if ($scope.sl_f[i].Remark === $scope.basedata.plan_list[k][pid].psplist[aid].sfeature.feature) {
              sport_selectoption.f = $scope.sl_f[i];
              $scope.setf();
              return true;
          };
        };
        $scope.basedata.plan_list[k][i].p_total_mets -= ($scope.basedata.plan_list[k][pid].psplist[aid].sfeature.cost/60) * $scope.basedata.plan_list[k][pid].psplist[aid].sfeature.mets ; 
